...
# Cross-platform path handling - automatically detect project root
# This works on Windows, macOS, and Linux
current_file = Path(__file__).resolve()
project_root = current_file.parent.parent.parent.parent  # Navigate to mditre root
...
def example_2_load_pickle_data():
    ...
    loader2 = DataLoaderRegistry.create_loader(
        "pickle", "path/to/data.pkl", config={"min_samples_per_subject": 3}
    )
...
def example_4_pytorch_datasets():
    ...
    train_loader = create_data_loader(X, y, mask, batch_size=16, shuffle=True)
    ...
    train_loader, val_loader = create_stratified_loaders(X, y, mask, train_ratio=0.8, batch_size=16)
...
def example_6_complete_workflow():
    ...
    train_loader, val_loader = create_stratified_loaders(
        X_train_val, y_train_val, train_ratio=0.8, batch_size=8
    )
